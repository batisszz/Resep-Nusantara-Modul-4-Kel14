...
export default function RecipeGrid({ recipes, onNavigate, favorites = [], onToggleFavorite }) {
  ...
  return (
    <section>
      ...
      <p className="text-center text-slate-500 max-w-2xl mx-auto mb-8">
        Temukan inspirasi Resep Nusantara favoritmu. Dari hidangan utama, aneka minuman, hingga camilan, semua ada di sini.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
        {recipes.map((recipe, index) => {
          // ✅ Pastikan makanan dan minuman tidak saling mempengaruhi
          const isFav = favorites.some(
            (f) => f.id === recipe.id && f.type === recipe.type
          );

          return (
            <div
              key={`${recipe.type}-${recipe.id}`} // ✅ key unik antar jenis
            ...
            >
              <div
                onClick={() => onNavigate && onNavigate("detail", recipe)}
              ...
              >
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation(); // biar gak klik ke detail
                    onToggleFavorite && onToggleFavorite(recipe);
                  }}
                  className="absolute top-3 right-3 z-50 bg-white/80 rounded-full p-2 hover:bg-white transition focus:outline-none"
                >
                  <Heart
                    className={`w-5 h-5 ${
